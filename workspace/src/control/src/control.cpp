 ...
 */
double cross(geometry_msgs::Point a, geometry_msgs::Point b)
{
 ...
 */
void personVelocityCallback(const geometry_msgs::Twist &msg)
{
 ...
 */
void personVisibleCallback(const std_msgs::Bool &msg)
{
...
int main(int argc, char **argv)
{
    ...
    // Subscribe topic
    ros::Subscriber sub_person_velocity = node.subscribe(param_sub_velocity, 1, personVelocityCallback);
    ros::Subscriber sub_person_visible = node.subscribe(param_sub_visible, 1, personVisibleCallback);
    ros::Subscriber sub_waypoint = node.subscribe(param_sub_waypoint, 1, waypointCallback);
    ros::Subscriber sub_odom = node.subscribe(param_sub_odom, 1, odomCallback);
    ros::Subscriber sub_imu = node.subscribe(param_sub_imu, 1, imuCallback);
    ...
    // Publish topics
    pub_takeoff = node.advertise<std_msgs::Empty>(param_pub_takeoff, 10);
    pub_land = node.advertise<std_msgs::Empty>(param_pub_land, 10);
    pub_velocity = node.advertise<geometry_msgs::Twist>(param_pub_velocity, 10);
